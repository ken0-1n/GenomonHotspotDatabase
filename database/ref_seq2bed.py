...
hCODING = open(outputPath + "/refGene.coding.tmp.bed","w")
...
for line in hIN:
    ...
    for i in range(0, len(starts) - 1):
        if (min(int(ends[i]), cdsStart) - int(starts[i]) > 0): 
            if (strand == "+"):
                # out 5PUTR
                print >> hCODING, chr +"\t"+ starts[i] +"\t"+ str(min(int(ends[i]), cdsStart)) +"\t"+ symbol +"("+ category +")"
            else:
                # out 3PUTR
                print >> hCODING, chr +"\t"+ starts[i] +"\t"+ str(min(int(ends[i]), cdsStart)) +"\t"+ symbol +"("+ category +")"
        ...
        if (min(int(ends[i]), cdsEnd) - max(int(starts[i]), cdsStart) > 0):
            # exon
            print >> hCODING, chr +"\t"+ str(max(int(starts[i]), cdsStart)) +"\t"+ str(min(int(ends[i]), cdsEnd)) +"\t"+ symbol +"("+ category+ ")"
        
        if (int(ends[i]) - max(cdsEnd, int(starts[i])) > 0):
            if (strand == "+"):
                print >> hCODING, chr +"\t"+ str(max(cdsEnd, int(starts[i]))) +"\t"+ ends[i] +"\t"+ symbol +"("+ category +")"
            else:
                print >> hCODING, chr +"\t"+ str(max(cdsEnd, int(starts[i]))) +"\t"+ ends[i] +"\t"+ symbol +"("+ category +")"
                
#     for i in range(1, len(starts) - 1):
#         print >> hCODING, chr +"\t"+ ends[i - 1] +"\t"+ starts[i] +"\t"+ symbol +"("+ category +")"
